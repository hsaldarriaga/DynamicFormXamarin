...
namespace DynamicForm3.iOS
{
    public class IOSDatabase
    {
                    ...
                    if (cp != null) // Es un campo
                    {
                        ...
                        else
                        {
                            ...
                            else if (p.PropertyType.Name.StartsWith("BO"))
                            {
                                ...
                                if (cus != null)
                                {
                                    var asso = p.GetCustomAttribute<DevExpress.Persistent.Base.AssociationAttribute>();
                                    if (asso != null)
                                        prop.Add("Association", asso.AssociationName);
                                    else
                                        prop.Add("Association", "null");
        ...
        {
            Document doc1 = db.GetExistingDocument("usuarios");
            if (doc1 == null)
            {
                Document doc = db.GetDocument("usuarios");
                if (doc != null)
                {
                    var dicc = new Dictionary<String,object>();
                    var docu = new Dictionary<String, String>{
                    {"rol","medico"},
                    {"contraseña","medi123"}
                    };
                    dicc.Add("medico1", docu);
                    var docu1 = new Dictionary<String, String>{
                    {"rol","fisico"},
                    {"contraseña","fis123"}
                    };
                    dicc.Add("fisico1", docu1);
                    var docu2 = new Dictionary<String, String>{
                    {"rol","recepcionista"},
                    {"contraseña","rec123"}
                    };
                    dicc.Add("recep1", docu2);
                    var docu3 = new Dictionary<String, String>{
                    {"rol","Asistente_Administrativo"},
                    {"contraseña","asis123"}
                    };
                    dicc.Add("asis1", docu3);
                    var docu4 = new Dictionary<String, String>{
                    {"rol","Equipo_Calidad"},
                    {"contraseña","eq123"}
                    };
                    dicc.Add("eqcal1", docu4);
                    var docu5 = new Dictionary<String, String>{
                    {"rol","nomatters"},
                    {"contraseña","12345"}
                    };
                    dicc.Add("admin", docu5);
                    doc.PutProperties(dicc);
                }
                return false;
            }
            return true;
        }
        public string iniciar_sesion(string user,string pass,out string mesagge_error) {
            Document doc = db.GetDocument("usuarios");
            if (doc == null)
            {
                mesagge_error = "Error Interno";
            }
            else { 
                var prop = doc.Properties;
                if (prop.ContainsKey(user))
                {
                    var value = (prop[user] as Newtonsoft.Json.Linq.JContainer).ToObject<Dictionary<string, string>>();
                    if (value["contraseña"].Equals(pass))
                    {
                        mesagge_error = "";
                        return value["rol"];
                    }
                    ...
                    {
                        mesagge_error = "Contraseña errada.";
                    }
                }
                else {
                    mesagge_error = "Usuario Erroneo";
                }
            }
            return null;
        }
